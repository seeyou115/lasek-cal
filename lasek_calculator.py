...
st.markdown("""
### 사용 방법
환자의 굴절 수치를 입력하시면 노모그램에 따라 조정된 절삭 수치와 총 절삭량을 자동으로 계산합니다.
""")
...
# WFO Ablation Depth 테이블 조회 함수
def get_ablation_depth(sph, cyl):
    """
    절삭 SPH와 CYL 값으로 총 절삭 깊이(micron)를 찾는 함수
    표에 정확한 값이 없으면 가장 가까운 값으로 근사
    """
    # WFO Ablation Depth 테이블 (SPH 행, CYL 열)
    ablation_table = {
...
if st.button("🔍 절삭 수치 계산", type="primary", use_container_width=True):
    ...
    # 총 절삭량 계산 및 표시
    st.markdown("---")
    st.markdown("### 🎯 총 절삭량 (Ablation Depth)")
    ...
    if ablation_depth is not None:
        ...
        with depth_col1:
            st.metric(
                ...
                help="절삭 SPH와 CYL 값에 따른 총 각막 절삭 깊이"
            )
...
<div style='text-align: center; color: gray; font-size: 0.9em;'>
    ...
    <p>📌 총 절삭량은 WFO Ablation Depth Table (OZ: 6.0mm, K: 43D, VD: 12)을 기준으로 합니다.</p>
</div>
